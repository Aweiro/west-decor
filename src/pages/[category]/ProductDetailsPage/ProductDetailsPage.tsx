...
import { productColors, ProductColorsType } from '../../../types/ProductColorsType';
...
export const ProductDetailsPage = () => {
  ...
  const { categorie: products, loading, error } = useCategoriesRTK(category as ProductsType);
  const {
        ...
        {normalizedItems.map(({ title, value }) => {
          ...
          return (
            <div className={styles['product-details__info-row']} key={value}>
              ...
              <p
                className={classNames(styles['product-details__info-description'], {
                  'body-text': !isSmall,
                })}
              >
                {value}
  ...
  const DescriptionBlock = ({
    ...
    return (
      <div className={classNames(styles['product-details__description-block'], className)}>
      ...
      <section className={`section ${styles['product-details__description']}`}>
        ...
        >
          {product.description.map((el, i) => (
            <div key={i} className={styles['product-details__description-content']}>
              <h4>{el.title}</h4>
